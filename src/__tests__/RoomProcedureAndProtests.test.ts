...
import { protestNoteLine, unresolvedProtestLines, unresolvedProtestMarker } from '../renderer/Services/ProtestNotes';
...
describe('the procedure is tournament state, not room state', () => {
  test('a tournament that configured nothing writes nothing, and reads back the same', async () => {
    const { default: Tournament } = await import('../renderer/DataModel/Tournament');
    const tournament = new Tournament();

    const written = tournament.toFileObject() as unknown as { YfData: Record<string, unknown> };

    expect(written.YfData.roomProcedure).toBeUndefined();
    expect(readRoomProcedure(written.YfData.roomProcedure)).toEqual(defaultRoomProcedure());
  });

  test('a configured procedure survives a write and a read', async () => {
    const { default: Tournament } = await import('../renderer/DataModel/Tournament');
    const tournament = new Tournament();
    ...
    expect(readRoomProcedure(written.YfData.roomProcedure)).toEqual({
      version: roomProcedureVersion,
      halves: true,
      halfLengthMinutes: 10,
      timeoutsPerTeam: 1,
    });
    // Never a QBJ concern: this is how a room runs a game, not how anybody scored one.
