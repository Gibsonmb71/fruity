...
import { openGameText, qbjMimeType, qbtcpExtensionKey } from 'qbsheet';
...
function makeSnapshot(): ITournamentSnapshot {
  ...
  return {
    ...
    rooms: [{ id: roomId, name: 'Room 204', accessToken: roomToken, pairingCode: '12345678', enabled: true }],
    assignments: [
      {
...
beforeEach(async () => {
  ...
  server = new TournamentServer({
    ...
    onFinalSubmission: () => {},
    // An empty allowlist means no cross-origin access at all, which is the safe default. A room
    // scoresheet on a public origin only works once a director has approved that origin.
...
describe('discovery', () => {
  test('announces the protocol, its version and what it supports', async () => {
    ...
    expect(body.qbj_version).toBe('2.1.1');
