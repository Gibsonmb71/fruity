...
/**
 * Restate a tournament's scoring rules as a descriptor.
 *
 * Total by construction. Unlike the MODAQ adapter there is no failure case, because every field is
 * copied from rules that already exist rather than mapped onto a foreign model. Configurations that
 * cannot be scored at all are a separate question; see `scorekeeperFormatProblems`.
 */
export default function scoringRulesToScorekeeperFormat(rules: ScoringRules): IScorekeeperFormat {
  return {
    ...
    bonus: {
      ...
      // ScoringRules.setUseBonuses already forces this false when bonuses are off, but the field is
      // publicly assignable, so don't rely on that having been the path taken.
