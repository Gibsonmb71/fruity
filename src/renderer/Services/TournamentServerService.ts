...
export default class TournamentServerService {
  ...
  /** Optional friendlier origin for room sheets and QR codes. Null means use the numeric address. */
  ...
  setPreferredRoomUrl(value: string): boolean {
    ...
    this.preferredRoomUrlValue = normalized;
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem(TournamentServerService.preferredRoomUrlStorageKey, normalized);
    }
    this.lastError = '';
    this.dataChangedReactCallback();
    return true;
  ...
  get roomLinkOrigin(): string {
    return resolveRoomLinkOrigin(this.preferredRoomUrlValue, this.selectedAddress);
  }
